	...
	public void perform(CommandContext context) {
		...
		if(you == null) {
			...
			for(String id : context.faction.getInvites()) {
				...
				msg.then(name + " ").color(ChatColor.WHITE).tooltip(TL.COMMAND_DEINVITE_CLICKTODEINVITE.format(name)).command("/" + FactionsPlugin.getInstance().configMain.getCommandBase().get(0) + " deinvite " + name);
			}
