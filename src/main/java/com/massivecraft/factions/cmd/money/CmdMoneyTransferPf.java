...
public class CmdMoneyTransferPf extends MoneyCommand {
	...
	public void perform(CommandContext context) {
		...
		if(success && FactionsPlugin.getInstance().conf().logging().isMoneyTransactions()) {
			FactionsPlugin.getInstance().log(ChatColor.stripColor(FactionsPlugin.getInstance().txt().parse(TL.COMMAND_MONEYTRANSFERPF_TRANSFER.toString(), context.fPlayer.getName(), VaultEconomy.moneyString(amount), from.describeTo(null), to.describeTo(null))));
		}
	}
