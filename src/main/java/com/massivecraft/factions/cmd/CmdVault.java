...
public class CmdVault extends FCommand {
	...
	public void perform(CommandContext context) {
		...
		if(number < 1) {
			...
			if(file == null) {
				context.sender.sendMessage(Lang.TITLE.toString() + Lang.VAULT_DOES_NOT_EXIST);
			} else {
				...
				context.sender.sendMessage(Lang.TITLE + Lang.EXISTING_VAULTS.toString().replaceAll("%p", context.fPlayer.getTag()).replaceAll("%v", sb.toString().trim()));
			}
			return;
		} // end listing vaults.
